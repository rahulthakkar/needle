...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self._shape) != prod(new_shape):
            raise ValueError(f"Product of current shape is not equal to the product of the new shape:"
                             f"self._shape={self._shape}, new_shape={new_shape}")
        if not self.is_compact():
            raise ValueError("self is not compact")
        new_strides = NDArray.compact_strides(new_shape)
        new_arr = NDArray.make(new_shape, new_strides, self.device, self._handle, self._offset) 
        return new_arr
    ...
    def permute(self, new_axes):
        ...
        if sorted(new_axes) != list(range(len(self._shape))):
            raise ValueError(f"Provided new_axes is not a valid permutation: new_axes={new_axes}")
        new_shape = tuple(self._shape[idx] for idx in new_axes)
        new_strides = tuple(self._strides[idx] for idx in new_axes)
        new_arr = NDArray.make(new_shape, new_strides, self.device, self._handle, self._offset) 
        return new_arr
    ...
    def broadcast_to(self, new_shape):
        ...
        if len(self._shape) != len(new_shape):
            raise AssertionError(f"Length of self._shape is not equal to new_shape: "
                                 f"len(self._shape)={len(self._shape)}, len(new_shape)={len(new_shape)}")
        if not all((new_dim==curr_dim or curr_dim==1) for new_dim, curr_dim in zip(new_shape, self._shape)):
            raise AssertionError(f"Array is not broadcastable to the new shape: "
                                 f"self._shape={self._shape}, new_shape={new_shape}")
        new_strides = tuple(0 if (new_dim != curr_dim) else curr_stride
                              for new_dim, curr_dim, curr_stride in zip(new_shape, self._shape, self._strides))
        new_arr = NDArray.make(new_shape, new_strides, self.device, self._handle, self._offset) 
        return new_arr
    ...
    def __getitem__(self, idxs):
        ...
        new_offset = self._offset + sum(s*idx.start for s, idx in zip(self._strides, idxs))
        new_strides = tuple(s*idx.step for s, idx in zip(self._strides, idxs))
        new_shape = tuple(math.ceil((idx.stop-idx.start)/idx.step) for idx in idxs)
        new_arr = NDArray.make(new_shape, new_strides, self.device, self._handle, new_offset) 
        return new_arr
    ...
    def flip(self, axes):
        ...
        """
        new_offset = sum(s * (dim-1) 
                          for i, (s, dim) in enumerate(zip(self._strides, self._shape)) 
                          if i in axes)
        new_strides = tuple(s if (i not in axes) else -s for i, s in enumerate(self._strides))
        out = NDArray.make(self._shape, new_strides, self.device, self._handle, new_offset) 
        return out.compact()
    ...
    def pad(self, axes):
        ...
        """
        assert len(self.shape) == len(axes), f"Need padding axes for {len(self.shape)} dimensions" 
        new_shape = tuple(sum(ax)+dim for ax, dim in zip(axes, self.shape))
        out = zeros(new_shape, device=self.device)
        index = tuple(slice(left, left+dim) for (left, _), dim in zip(axes, self.shape))
        out[index] = self
        return out
